...
// Helper to Format Time correctly from Google Sheets Date Object
function _formatTime(val) {
  if (!val) return "";
  // If it's already a string, return it.
  if (typeof val === 'string') return val;
  // If it's a Date object (Sheets default for time), format to HH:mm
...
function _sendBrandedEmail(to, subject, title, bodyContent, attachmentBlob) {
  ...
  var options = { htmlBody: htmlBody, name: "Event Bisdig System", noReply: true };
  if (attachmentBlob) options.attachments = [attachmentBlob];

  try { GmailApp.sendEmail(to, subject, textBody, options); } catch(e) { MailApp.sendEmail(to, subject, textBody, options); }
}
...
function getEvents() { 
  ...
  return dataRows.map(function(row) { 
    ...
    obj.date = row[3]; 
    obj.time = _formatTime(row[4]); // FIX: Format Time Here 
    obj.location = row[5]; 
    obj.price = row[6]; 
    obj.category = row[7]; 
    obj.bannerUrl = row[8]; 
    obj.maxParticipants = row[9]; 
    obj.currentParticipants = row[10]; 
    ...
    obj.enableTicketScanner = row[15] === true || String(row[15]).toUpperCase() === "TRUE"; 
    // Added autoSendCertificate check (col 17 / index 16)
    obj.autoSendCertificate = row[16] === true || String(row[16]).toUpperCase() === "TRUE";
    return obj; 
  }); 
}
...
function createEvent(data) { 
  ...
  var bannerUrl = ""; var thumbnailUrl = ""; 
  var folder = _getAdminFolder(); 
  if (data.bannerBase64) { try { var blob = Utilities.newBlob(Utilities.base64Decode(data.bannerBase64), "image/jpeg", "banner_" + id); bannerUrl = "https://lh3.googleusercontent.com/d/" + folder.createFile(blob).getId(); } catch (e) { } } 
  if (data.thumbnailBase64) { try { var blob = Utilities.newBlob(Utilities.base64Decode(data.thumbnailBase64), "image/jpeg", "thumb_" + id); thumbnailUrl = "https://lh3.googleusercontent.com/d/" + folder.createFile(blob).getId(); } catch (e) { } } 
  var certConfig = data.certificateConfig || null; 
  if (certConfig && data.certBackgroundBase64) { try { var blob = Utilities.newBlob(Utilities.base64Decode(data.certBackgroundBase64), "image/png", "cert_bg_" + id); var file = folder.createFile(blob); file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW); certConfig.backgroundUrl = "https://lh3.googleusercontent.com/d/" + file.getId(); } catch(e) {} } 
  var formFieldsJson = data.formFields ? JSON.stringify(data.formFields) : "[]"; 
  var certConfigJson = certConfig ? JSON.stringify(certConfig) : ""; 
  
  // Appending new row with autoSendCertificate
  sheet.appendRow([ id, data.title, data.description, data.date, "'" + data.time, data.location, data.price, data.category, bannerUrl, data.maxParticipants, 0, true, formFieldsJson, certConfigJson, thumbnailUrl, data.enableTicketScanner, data.autoSendCertificate ]); 
  return { id: id }; 
}
...
function updateEvent(data) { 
  ...
  for (var i = 1; i < rows.length; i++) { 
    if (rows[i][0] == data.id) { 
      var bannerUrl = rows[i][8]; var thumbnailUrl = rows[i][14] || ""; 
      var folder = _getAdminFolder(); 
      ...
      sheet.getRange(i+1, 5).setValue("'" + data.time); // Force string for time
      ...
      // Update new fields if they exist, create if not
      if (sheet.getLastColumn() < 16) sheet.getRange(i+1, 16).setValue(data.enableTicketScanner); else sheet.getRange(i+1, 16).setValue(data.enableTicketScanner);
      if (sheet.getLastColumn() < 17) sheet.getRange(i+1, 17).setValue(data.autoSendCertificate); else sheet.getRange(i+1, 17).setValue(data.autoSendCertificate);
...
function registerEventParticipant(data) { /*...*/ var eSheet = _getSheet("Events"); var events = eSheet.getDataRange().getValues(); var eventRowIndex = -1; var eventTitle = ""; for(var i=1; i<events.length; i++) { if(events[i][0] == data.eventId) { eventRowIndex = i; eventTitle = events[i][1]; break; } } if (eventRowIndex == -1) throw new Error("Event not found"); var rSheet = _getSheet("Registrations"); var rRows = rSheet.getDataRange().getValues(); for(var i=1; i<rRows.length; i++) { if(rRows[i][1] == data.eventId && rRows[i][4].toString().toLowerCase() == data.email.toString().toLowerCase()) throw new Error("Email ini sudah terdaftar untuk acara tersebut."); } var proofUrl = ""; var userFolder = _getUserFolder(); if (data.proofBase64) { try { var blob = Utilities.newBlob(Utilities.base64Decode(data.proofBase64.split(',')[1] || data.proofBase64), "image/jpeg", "proof_" + data.email); proofUrl = userFolder.createFile(blob).getUrl(); } catch(e) {} } var finalCustomData = {}; if (data.customData) { try { var rawData = data.customData; Object.keys(rawData).forEach(function(key) { var val = rawData[key]; if (val && typeof val === 'object' && val.isCustomFile && val.data) { try { var fileBlob = Utilities.newBlob(Utilities.base64Decode(val.data), val.mimeType || "application/octet-stream", "file_" + val.fileName); var uploadedFile = userFolder.createFile(fileBlob); uploadedFile.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW); finalCustomData[key] = uploadedFile.getUrl(); } catch (fileErr) { finalCustomData[key] = "Error Uploading File: " + val.fileName; } } else { finalCustomData[key] = val; } }); } catch (e) { finalCustomData = data.customData; } } var customDataJson = JSON.stringify(finalCustomData); rSheet.appendRow([ Utilities.getUuid(), data.eventId, eventTitle, data.name, data.email, proofUrl, "PENDING", new Date().toISOString(), customDataJson, "NOT_USED", "" ]); var current = Number(events[eventRowIndex][10]) || 0; eSheet.getRange(eventRowIndex + 1, 11).setValue(current + 1); var body = `<p>Halo <strong>${data.name}</strong>, 👋</p><p>Terima kasih telah mendaftar di acara <strong>"${eventTitle}"</strong>.</p><p>Data pendaftaran dan bukti pembayaran Anda telah kami terima. Tim kami sedang melakukan verifikasi.</p><p>Mohon cek email Anda secara berkala untuk info selanjutnya.</p>`; _sendBrandedEmail(data.email, "Pendaftaran Diterima", "MENUNGGU VERIFIKASI", body); return { status: "PENDING" }; }
