...
function _setupPermissions() {
  ...
  var tempSlide = SlidesApp.create("TEMP_AUTH_CHECK"); 
...
function signupUser(data) { 
  ...
  var rawHash = Utilities.computeDigest(Utilities.DigestAlgorithm.MD5, password, Utilities.Charset.UTF_8); 
  var txtHash = rawHash.map(function(b) { return (b < 0 ? b + 256 : b).toString(16).padStart(2,'0'); }).join(""); 
  sheet.appendRow([ Utilities.getUuid(), email, txtHash, name, new Date().toISOString() ]); 
  return { created: true }; 
}
...
function loginUser(data) { 
  ...
  if (ADMIN_EMAILS.indexOf(found.email) > -1) { 
    ...
    var body = `<p>Halo Admin,</p><p>Seseorang mencoba mengakses panel admin. Gunakan kode berikut untuk melanjutkan:</p><h1 style="color:#2B427A; letter-spacing: 5px; font-size: 32px;">${otp}</h1><p style="color: #64748b; font-size: 12px;">Kode ini hanya berlaku selama 5 menit.</p>`; 
    _sendBrandedEmail(found.email, "Kode Verifikasi Admin", "LOGIN ADMIN", body); 
...
function requestOtp(data) { 
  ...
  var title = "VERIFIKASI AKUN"; 
  var subject = "Kode Verifikasi Event Bisdig"; 
  var intro = "gunakan kode verifikasi berikut untuk masuk ke akun Anda:";
  if (data.type === 'RESET') { 
    title = "RESET PASSWORD"; 
    subject = "Permintaan Reset Password"; 
    intro = "kami menerima permintaan untuk mereset password Anda. Gunakan kode ini:"; 
  } 
  var body = `<p>Halo,</p><p>Demi keamanan, ${intro}</p><h1 style="color:#2B427A; letter-spacing: 5px; font-size: 32px;">${otp}</h1><p>Jangan berikan kode ini kepada siapa pun, termasuk pihak HMP.</p>`; 
  _sendBrandedEmail(email, subject, title, body); 
  return { sent: true }; 
}
...
function loginWithOtp(data) { 
  ...
  if (!cached || String(cached) !== String(data.otp)) throw new Error("OTP salah atau kadaluarsa."); 
...
function resetUserPassword(data) { 
  ...
  var otp = data.otp; 
  if (!email || !newPassword || !otp) throw new Error("Data tidak lengkap."); 
  var cached = CacheService.getScriptCache().get("OTP_" + email); 
  if (!cached || String(cached) !== String(otp)) throw new Error("Kode OTP salah atau kadaluarsa."); 
  ...
  for (var i = 1; i < rows.length; i++) { 
    if (String(rows[i][1]).toLowerCase() === email) { 
      var rawHash = Utilities.computeDigest(Utilities.DigestAlgorithm.MD5, newPassword, Utilities.Charset.UTF_8); 
      var txtHash = rawHash.map(function(b) { return (b < 0 ? b + 256 : b).toString(16).padStart(2,'0'); }).join(""); 
  ...
  if (!found) throw new Error("Email tidak terdaftar."); 
  CacheService.getScriptCache().remove("OTP_" + email); 
  return { success: true }; 
}

function _sendBrandedEmail(to, subject, title, bodyContent, attachmentBlob) { var colorPrimary = "#2B427A"; var colorAccent = "#DFFF00"; var htmlBody = ` <!DOCTYPE html> <html> <head> <meta charset="utf-8"> <meta name="viewport" content="width=device-width, initial-scale=1.0"> <title>${subject}</title> </head> <body style="margin: 0; padding: 0; background-color: #F8FAFC; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;"> <table width="100%" border="0" cellpadding="0" cellspacing="0" style="background-color: #F8FAFC; padding: 20px 0;"> <tr> <td align="center"> <table width="600" border="0" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); border: 1px solid #e2e8f0; max-width: 90%;"> <tr> <td align="center" style="background-color: ${colorPrimary}; padding: 30px 20px;"> <h1 style="color: ${colorAccent}; margin: 0; font-size: 24px; font-weight: 900; text-transform: uppercase; letter-spacing: 1px;">EVENT BISDIG</h1> <p style="color: #E2E8F0; margin: 5px 0 0; font-size: 12px; font-weight: 500;">Himpunan Mahasiswa Bisnis Digital UPY</p> </td> </tr> <tr> <td style="padding: 40px 30px;"> <h2 style="color: ${colorPrimary}; font-size: 20px; font-weight: 800; margin: 0 0 25px 0; text-transform: uppercase; border-bottom: 2px solid #F1F5F9; padding-bottom: 15px;">${title}</h2> <div style="color: #334155; font-size: 16px; line-height: 1.6;"> ${bodyContent} </div> </td> </tr> <tr> <td align="center" style="background-color: #F1F5F9; padding: 25px; border-top: 1px solid #E2E8F0;"> <p style="margin: 0; font-size: 12px; color: #64748b; font-weight: 600;">&copy; ${new Date().getFullYear()} HMP Bisnis Digital UPY</p> <p style="margin: 5px 0 0; font-size: 11px; color: #94A3B8;">Pesan ini dikirim secara otomatis. Mohon tidak membalas email ini.</p> </td> </tr> </table> </td> </tr> </table> </body> </html> `; var textBody = bodyContent.replace(/<[^>]*>/g, "").trim(); textBody += "\n\nTerima kasih,\nHMP Bisnis Digital UPY"; var options = { htmlBody: htmlBody, name: "Event Bisdig Official", noReply: true }; if (attachmentBlob) { options.attachments = [attachmentBlob]; } try { GmailApp.sendEmail(to, subject, textBody, options); } catch(e) { MailApp.sendEmail(to, subject, textBody, options); } }
function _generateEmailButton(url, text) { return ` <table border="0" cellpadding="0" cellspacing="0" style="margin: 30px 0; width: 100%;"> <tr> <td align="center"> <a href="${url}" target="_blank" style="background-color: #0B1CDE; color: #ffffff; font-size: 16px; font-family: Helvetica, Arial, sans-serif; text-decoration: none; border-radius: 8px; padding: 16px 32px; display: inline-block; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; box-shadow: 0 4px 6px rgba(11, 28, 222, 0.2);"> ${text} </a> </td> </tr> </table> `; }
...
function updateRegistrationStatus(data) { var sheet = _getSheet("Registrations"); var rows = sheet.getDataRange().getValues(); for (var i = 1; i < rows.length; i++) { if (rows[i][0] == data.id) { sheet.getRange(i + 1, 7).setValue(data.status); var email = rows[i][4]; var name = rows[i][3]; var evtTitle = rows[i][2]; var isApproved = data.status === 'APPROVED'; var statusTitle = isApproved ? 'PENDAFTARAN DISETUJUI' : 'PENDAFTARAN DITOLAK'; var body = ""; if (isApproved) { var baseUrl = data.baseUrl || "https://bisdig.upy.ac.id/hmp/"; var cleanBaseUrl = baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl; var ticketLink = cleanBaseUrl + "/#/ticket-view/" + data.id; body = `<p>Halo <strong>${name}</strong>,</p><p>Selamat! Pembayaran Anda telah berhasil kami verifikasi.</p><p>Tiket Anda untuk acara <strong>"${evtTitle}"</strong> sekarang berstatus <span style="color:green; font-weight:bold;">VALID / APPROVED</span>.</p><p>Silakan klik tombol di bawah ini untuk melihat E-Ticket Anda. Jangan lupa simpan QR Code untuk proses check-in saat acara berlangsung.</p>${_generateEmailButton(ticketLink, "LIHAT E-TICKET SAYA")}<br/><p style="font-size:12px; color:gray;">Atau login ke dashboard peserta untuk melihat detail lengkap.</p><p>Sampai jumpa di acara!</p>`; } else { body = `<p>Halo <strong>${name}</strong>,</p><p>Mohon maaf, kami menginformasikan bahwa pendaftaran Anda untuk acara <strong>"${evtTitle}"</strong> belum dapat kami setujui.</p><p>Hal ini mungkin disebabkan oleh salah satu alasan berikut:</p><ul><li>Bukti pembayaran tidak terbaca atau nominal tidak sesuai.</li><li>Kuota peserta telah terpenuhi sebelum Anda menyelesaikan pembayaran.</li><li>Data pendaftaran tidak valid.</li></ul><p>Jika Anda merasa ini adalah kesalahan, silakan segera hubungi Admin atau Contact Person acara kami.</p>`; } try { _sendBrandedEmail(email, "Update Status Pendaftaran: " + evtTitle, statusTitle, body); } catch(e){} return { status: data.status }; } } throw new Error("Registration not found"); }
...
function sendCertificate(data) { var sheet = _getSheet("Registrations"); var rows = sheet.getDataRange().getValues(); var baseUrl = data.baseUrl || "https://bisdig.upy.ac.id/hmp/"; var cleanBaseUrl = baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl; var certLink = cleanBaseUrl + "/#/certificate/" + data.id; for(var i=1; i<rows.length; i++) { if(rows[i][0] == data.id) { var name = rows[i][3]; var evtTitle = rows[i][2]; var body = `<p>Halo <strong>${name}</strong>,</p><p>Terima kasih banyak atas partisipasi dan antusiasme Anda dalam acara <strong>"${evtTitle}"</strong>.</p><p>Kami sangat mengapresiasi kehadiran Anda. Sebagai bentuk penghargaan, berikut kami lampirkan <strong>E-Sertifikat</strong> resmi keikutsertaan Anda.</p><p>Kami berharap ilmu dan pengalaman yang didapatkan dapat bermanfaat bagi pengembangan diri dan karir Anda di masa depan.</p><p>Silakan unduh sertifikat Anda melalui tombol di bawah ini:</p>${_generateEmailButton(certLink, "DOWNLOAD SERTIFIKAT")}<br/><p style="font-size:12px; color:#64748b; margin-top:20px;">Jika tombol tidak berfungsi, silakan login ke dashboard peserta untuk mengunduh manual.</p>`; _sendBrandedEmail(rows[i][4], "E-Sertifikat Peserta: " + evtTitle, "SERTIFIKAT ANDA", body); return { sent: true }; } } throw new Error("Not found"); }
function sendBulkCertificates(data) { var sheet = _getSheet("Registrations"); var rows = sheet.getDataRange().getValues(); var idsToProcess = data.ids || []; var successCount = 0; var failCount = 0; var idMap = {}; var baseUrl = data.baseUrl || "https://bisdig.upy.ac.id/hmp/"; var cleanBaseUrl = baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl; for(var i=1; i<rows.length; i++) { idMap[rows[i][0]] = i; } idsToProcess.forEach(function(id) { var rowIndex = idMap[id]; if(rowIndex) { var row = rows[rowIndex]; if(row[6] === 'APPROVED') { try { var certLink = cleanBaseUrl + "/#/certificate/" + id; var name = row[3]; var evtTitle = row[2]; var body = `<p>Halo <strong>${name}</strong>,</p><p>Terima kasih telah menjadi bagian dari kesuksesan acara <strong>"${evtTitle}"</strong>.</p><p>Berikut adalah <strong>E-Sertifikat</strong> resmi Anda sebagai tanda partisipasi.</p><p>Semoga bermanfaat dan sampai jumpa di acara kami selanjutnya!</p>${_generateEmailButton(certLink, "DOWNLOAD SERTIFIKAT")}`; _sendBrandedEmail(row[4], "E-Sertifikat Peserta: " + evtTitle, "SERTIFIKAT ANDA", body); successCount++; } catch(e) { failCount++; } } } }); return { sent: successCount, failed: failCount }; }
