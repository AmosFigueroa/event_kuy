    ...
    <div className="group bg-white rounded-xl border-2 border-[#2B427A] shadow-[6px_6px_0px_0px_#2B427A] hover:shadow-[8px_8px_0px_0px_#0B1CDE] hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col h-full relative">
      <div className="relative h-64 bg-gray-200 overflow-hidden border-b-2 border-[#2B427A]">
        ...
        {event.status === 'EXTENDED' && (
            <div className="absolute top-0 left-0 bg-[#0B1CDE] text-white px-3 py-1 text-[10px] font-black uppercase tracking-wider border-r-2 border-b-2 border-white shadow-md">
                DIPERPANJANG
            </div>
        )}
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        {/* If Coming Soon, blur details and show clean minimalist look */}
        {isComingSoon ? (
            <div className="flex-1 flex flex-col items-center justify-center py-6">
                <div className="bg-blue-50 border-2 border-[#0B1CDE] px-4 py-2 rounded-xl text-[#0B1CDE] font-black uppercase tracking-widest text-sm flex items-center gap-2">
                    <Clock className="w-4 h-4" /> SEGERA HADIR
                </div>
                <p className="text-gray-400 text-xs font-bold mt-4 text-center">Detail acara akan segera diumumkan.</p>
            </div>
        ) : (
            <>
