...
const AdminDashboard: React.FC = () => {
  ...
  const [isMobileSidebarOpen, setIsMobileSidebarOpen] = useState(false); // New state for mobile toggle
  ...
  const handleBulkSendCertificates = () => {
      ...
      const eligibleRegistrations = registrations.filter(r => r.eventId === selectedEventFilter && r.status === RegistrationStatus.APPROVED);
      ...
      showConfirm(
          ...
          `Akan mengirim ${eligibleRegistrations.length} email sertifikat. Lanjutkan?`,
          async () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans relative overflow-x-hidden">
      ...
      {/* Export Modal */}
      {showExportModal && <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowExportModal(false)}><div className="bg-white rounded-xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}><h3 className="font-black text-[#2B427A] mb-4">Export Data</h3><div className="space-y-4"><div><label className="text-xs font-bold text-gray-500">Pilih Acara</label><select value={selectedEventFilter} onChange={e => setSelectedEventFilter(e.target.value)} className="w-full border p-2 rounded font-bold"><option value="ALL">Semua</option>{events.map(e => <option key={e.id} value={e.id}>{e.title}</option>)}</select></div><button onClick={handleExportData} disabled={exportLoading} className="w-full py-3 bg-green-600 text-white font-black rounded-lg flex items-center justify-center gap-2">{exportLoading ? <Loader className="w-4 h-4 animate-spin"/> : <Download className="w-4 h-4"/>} DOWNLOAD CSV</button></div></div></div>}

      {/* MOBILE OVERLAY */}
      {isMobileSidebarOpen && (
          <div className="fixed inset-0 bg-black/50 z-20 md:hidden backdrop-blur-sm transition-opacity" onClick={() => setIsMobileSidebarOpen(false)}></div>
      )}
            ...
            {activeTab === 'settings' && (
               <div className="flex flex-col md:flex-row h-auto md:h-[calc(100vh-140px)] bg-white rounded-xl border-2 border-[#2B427A] shadow-md overflow-hidden">
                   <div className="w-full md:w-64 bg-gray-50 border-r-2 border-gray-200 flex flex-col">
                       <div className="p-4 border-b border-gray-200"><h3 className="font-black text-[#2B427A] uppercase">Menu</h3></div>
                       <nav className="flex-row md:flex-col flex overflow-x-auto md:overflow-visible p-2 md:p-4 gap-2">
                           <button onClick={() => setSettingsTab('payment')} className={`flex-shrink-0 px-4 py-2 rounded-lg font-bold text-xs ${settingsTab === 'payment' ? 'bg-[#2B427A] text-white' : 'text-gray-600'}`}>Pembayaran</button>
                           <button onClick={() => setSettingsTab('certificate')} className={`flex-shrink-0 px-4 py-2 rounded-lg font-bold text-xs ${settingsTab === 'certificate' ? 'bg-[#2B427A] text-white' : 'text-gray-600'}`}>Sertifikat</button>
                       </nav>
                   </div>
                   <div className="flex-1 overflow-y-auto p-6 bg-white">
                       {settingsTab === 'payment' && (
                          <div className="space-y-6">
                             <div className="flex justify-between items-center border-b pb-2"><h3 className="font-black text-[#2B427A]">REKENING & QRIS</h3><button onClick={handleSavePaymentSettings} disabled={savingPayment} className="px-4 py-2 bg-[#0B1CDE] text-white rounded font-bold text-xs">{savingPayment ? '...' : 'SIMPAN'}</button></div>
                             <div className="grid md:grid-cols-2 gap-6">
                                <div><h4 className="text-xs font-bold text-gray-500 mb-2">DAFTAR BANK</h4>{paymentSettings.bankAccounts.map((acc, idx) => (<div key={idx} className="bg-gray-50 p-2 rounded mb-2 border flex justify-between items-center"><div><div className="font-black text-sm">{acc.bankName}</div><div className="text-xs font-mono">{acc.accountNumber}</div></div><button onClick={()=>{const u=paymentSettings.bankAccounts.filter((_,i)=>i!==idx);setPaymentSettings({...paymentSettings,bankAccounts:u})}} className="text-red-500"><Trash2 className="w-3 h-3"/></button></div>))}<div className="mt-4 bg-blue-50 p-3 rounded border border-blue-100"><input placeholder="Bank" value={tempAccount.bankName} onChange={e=>setTempAccount({...tempAccount,bankName:e.target.value})} className="w-full mb-2 p-1 text-xs border rounded"/><input placeholder="No. Rek" value={tempAccount.accountNumber} onChange={e=>setTempAccount({...tempAccount,accountNumber:e.target.value})} className="w-full mb-2 p-1 text-xs border rounded"/><input placeholder="Atas Nama" value={tempAccount.accountHolder} onChange={e=>setTempAccount({...tempAccount,accountHolder:e.target.value})} className="w-full mb-2 p-1 text-xs border rounded"/><button onClick={()=>{if(tempAccount.bankName){setPaymentSettings({...paymentSettings,bankAccounts:[...paymentSettings.bankAccounts,{...tempAccount,id:Date.now().toString()}]});setTempAccount({id:'',bankName:'',accountNumber:'',accountHolder:''})}}} className="w-full bg-[#2B427A] text-white py-1 rounded text-xs font-bold">TAMBAH</button></div></div>
                                <div><h4 className="text-xs font-bold text-gray-500 mb-2">UPLOAD QRIS</h4><div className="border-2 border-dashed rounded p-4 text-center"><input type="file" onChange={e=>setQrisFile(e.target.files?.[0]||null)} className="w-full text-xs" />{paymentSettings.qrisUrl && <img src={paymentSettings.qrisUrl} className="h-32 mx-auto mt-2 object-contain"/>}</div></div>
                             </div>
                          </div>
                       )}
                       {settingsTab === 'certificate' && <div className="text-center text-gray-400 py-10">Editor Sertifikat Default (Desktop Only)</div>}
                   </div>
               </div>
            )}
